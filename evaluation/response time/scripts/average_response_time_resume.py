...
def calculate_api_response_time_post(api_url, simultaneous_requests, total_requests, filename, message, headers):
    ...
    with ThreadPoolExecutor(max_workers=simultaneous_requests) as executor:
        request_data = [{"filename": message[i % len(message)]} for i in range(total_requests)]
        ...
        for future in as_completed(futures):
            try:
                ...
                log(filename, f"Request completed in {response_time:.2f} [{status_code}]")
...
if __name__ == "__main__":
    api_url = "http://108.59.81.219:8080/resume-service/analyze"
    message = [
        "example.pdf",
        "example-injection.pdf"
    ]
    headers = {"Content-Type": "application/json"}
    simultaneous_requests = 4
    total_requests = simultaneous_requests * 25
    filename = "res/resume-response-time.log"
