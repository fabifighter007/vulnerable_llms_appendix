...
def calculate_api_response_time_post(api_url, simultaneous_requests, total_requests, filename, message, headers):
    ...
    with ThreadPoolExecutor(max_workers=simultaneous_requests) as executor:
        request_data = [{"message": message[i % len(message)]} for i in range(total_requests)]
...
if __name__ == "__main__":
    api_url = "http://108.59.81.219:8080/ackermann-service/chat"
    message = [
        "Hi!",
        "Whats your name?",
    ...
    filename = "res/ackermann-response-time.log"
